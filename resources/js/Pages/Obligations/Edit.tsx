...
export default function ObligationsEdit({ obligation, instruments = [] }: Props) {
    const { data, setData, put, processing, errors } = useForm<FormData>({
        instrument_id: String(obligation.instrument_id ?? ''),
        reference: obligation.reference ?? '',
        title: obligation.title ?? '',
        description: obligation.description ?? '',
        due_basis: obligation.due_basis ?? '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('obligations.update', obligation.id));
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit Obligation: ${obligation.title}`} />

            <PageHeader
                title={`Edit Obligation: ${obligation.title}`}
                breadcrumb={[
                    { label: 'Obligations', href: route('obligations.index') },
                    { label: obligation.reference, href: route('obligations.show', obligation.id) },
                    { label: 'Edit' },
                ]}
            />
            ...
            <form onSubmit={handleSubmit} noValidate>
                ...
                <div className="flex items-center justify-end gap-3 mt-2">
                    <Link href={route('obligations.show', obligation.id)}>

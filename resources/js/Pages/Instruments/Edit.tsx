...
interface Instrument {
    ...
    instrument_type_id: string | number;
    nature_id: string | number;
    status_id: string | number;
    area_of_focus_id: string | number;
    risk_rating_id: string | number;
...
export default function InstrumentsEdit({
    instrument,
    regulators = [],
    instrument_types = [],
    natures = [],
    ...
    const { data, setData, put, processing, errors } = useForm<FormData>({
        source_title: instrument.source_title ?? '',
        regulator_id: String(instrument.regulator_id ?? ''),
        instrument_type_id: String(instrument.instrument_type_id ?? ''),
        nature_id: String(instrument.nature_id ?? ''),
        status_id: String(instrument.status_id ?? ''),
        area_of_focus_id: String(instrument.area_of_focus_id ?? ''),
        risk_rating_id: String(instrument.risk_rating_id ?? ''),
        risk_rating_explain: instrument.risk_rating_explain ?? '',
        objectives: instrument.objectives ?? '',
        commercial_bank_relevance: instrument.commercial_bank_relevance ?? '',
        commercial_bank_compliance_context: instrument.commercial_bank_compliance_context ?? '',
        applicability: instrument.applicability ?? '',
        date_issue: instrument.date_issue ?? '',
        date_commence: instrument.date_commence ?? '',
        link_url: instrument.link_url ?? '',
    });
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit Instrument: ${instrument.source_title}`} />

            <PageHeader
                title={`Edit Instrument: ${instrument.source_title}`}
                breadcrumb={[
                    { label: 'Library', href: route('instruments.index') },
                    { label: 'Instruments', href: route('instruments.index') },
                    { label: instrument.source_title, href: route('instruments.show', instrument.id) },
                    { label: 'Edit' },
                ]}
            />
            ...
            <form onSubmit={handleSubmit} noValidate>
                ...
                <div className="flex items-center justify-end gap-3 mt-2">
                    <Link href={route('instruments.show', instrument.id)}>

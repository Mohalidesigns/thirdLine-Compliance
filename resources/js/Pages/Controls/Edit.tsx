...
export default function ControlsEdit({ control, types, natures, frequencies, statuses, obligations }: Props) {
    const currentLinkedIds = control.linked_obligations.map((o) => o.id);
    ...
    const { data, setData, put, processing, errors } = useForm<FormData>({
        ...
        linked_obligation_ids: currentLinkedIds,
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('controls.update', control.id));
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Edit Control — ${control.reference}`} />

            <PageHeader
                title={`Edit Control — ${control.reference}`}
                breadcrumb={[
